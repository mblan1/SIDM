...
function checkConnection(): void {
    ...
    const timeout = setTimeout(() => {
        ...
        setStatus('bad', 'No response from SIDM',
            'Is SIDM running, and did you run `SIDM.App --register-hosts`? Firefox uses the sidm@snw.dev extension id which is already wired up.');
    }, 4000);
    ...
    port.postMessage({
        type: 'hello',
        clientName: 'SIDM-Firefox-Extension-Options',
